...
namespace TreesReborn
{
    ...
    {
        public enum AlwaysEnabledToggle
        {
            Enabled = 1
        }
        ...
        internal static ConfigEntry<AlwaysEnabledToggle> BindHiddenForceEnabledSyncLocker(this ConfigFile configFile, ConfigSync serverSyncInstance, string group, string name)
        {
            ConfigEntry<AlwaysEnabledToggle> configEntry = configFile.Bind(group, name, AlwaysEnabledToggle.Enabled, HiddenDisplay("It's required that the mod is installed on the server and all clients. Changing this setting does nothing, ServerSync is always enabled."));
            configEntry.Value = AlwaysEnabledToggle.Enabled;
